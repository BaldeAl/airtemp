      ...
      <div className="card-cartoon p-6 sticky top-28">
        <div className="text-center py-6">
          ...
          <h3 className="text-xl font-extrabold text-[#2D3436] dark:text-white mb-2">
            {t("booking_form.reservationPending")}
          </h3>
          <p className="text-[#636E72] dark:text-[#B2BEC3] text-sm mb-2">
            {nights}{" "}
            {nights !== 1 ? t("booking_form.nights") : t("booking_form.night")}{" "}
            {t("booking_form.nightsAt")} {place.name}
          </p>
          <p className="text-lg font-extrabold text-[#FF6B6B] mb-4">
            {totalPrice}€ {t("booking_form.total").toLowerCase()}
          </p>
  ...
  return (
    <div className="card-cartoon p-6 sticky top-28">
      <div className="flex items-baseline gap-2 mb-6">
        ...
        <span className="text-[#B2BEC3] font-medium">
      ...
      <form onSubmit={handleBook} className="space-y-4">
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className="text-xs font-bold text-[#636E72] dark:text-[#B2BEC3] mb-1.5 block uppercase tracking-wide">
              {t("booking_form.checkIn")}
            </label>
            <input
              type="date"
              value={checkIn}
        ...
        >
          {isLoading
            ? t("booking_form.booking")
            : availabilityStatus === "unavailable"
              ? t("booking_form.notAvailable")
