  ...
  const handleSubmitReview = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setComment("");
        setHasReviewed(true);
        setFormMessage(t("reviews.reviewSubmitted"));
        setFormError(false);
      } else {
        setFormMessage(data.message || t("reviews.failedSubmit"));
        setFormError(true);
      }
    } catch {
      setFormMessage(t("auth.networkError"));
      setFormError(true);
    } finally {
      setSubmitting(false);
    }
  ...
  const handleSubmitReply = async (reviewId) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        toast.error(data.message || t("reviews.replyFailed"));
      }
    } catch {
      ...
      {reviews.length === 0 ? (
        ...
        <>
          ...
          {reviews.length > 6 && !showAll && (
            ...
            >
              {t("reviews.showAll")} {reviews.length} {t("reviews.reviewsWord")}
            </button>
          )}
        </>
      )}
